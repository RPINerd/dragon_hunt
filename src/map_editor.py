...
def refresh_map():
    ...
    g.create_norm_box(
        ...
        ((scroll_length * portalsize) / mapsize_x, 4),
        inner_color="ep_blue",
    )
    g.create_norm_box((scroll_length - 1, 0), (4, scroll_length))
    g.create_norm_box(
        (scroll_length - 1, (scroll_length * portal_y) / mapsize_y),
...
def set_tile(x, y, name):
    ...
    if tile_prefix == "items" and cur_mode != "remove":
        ...
        if pygame.key.get_mods() & pygame.KMOD_SHIFT:
            ...
            copy_tile(cur_map.field[y][x], cur_map.field[cur_tilecopy_y][cur_tilecopy_x])
    else:
        temp_name = name
        for line_num in range(len(cur_map.field[y][x].onload)):
        ...
        if tail != "CVS":
            ...
            for tilename in files:
                ...
                if root != g.mod_directory + "/images/tiles/":
                    ...
                    tiles[tilename] = pygame.image.load(root + "/" + tilename).convert_alpha()
    get_walk_values()
    ...
    for tmpy in range(len(cur_map.field)):
        if len(cur_map.field[tmpy]) > int(x):
            ...
            for tmpx in range(int(x) - len(cur_map.field[tmpy])):
                ...
                cur_map.field[tmpy][len(cur_map.field[tmpy]) - 1].onload.append("walk(1)")

    global portal_x
    global portal_y
    portal_x = 0
    portal_y = 0
    line = "Reset border and background for new map size ? " "(This will clear the contents of the map.)"
    if ask_yesno(line):
        set_border(border)
        set_bkg(background)
    refresh_map()
...
def select_from_list(input_array, act_as_menu=False, return_pos=False, extra_wide=False):
    cur_pos = 0
...
def map_menu():
    list_of_opts = ["Map Mode", "Map Size", "Background", "Border", "Monsters", "Battle Background"]
    tmp = select_from_list(list_of_opts, True)
    if tmp == -1:
        refresh_map()
        return
    if tmp == "Map Mode":
        list_of_opts = [
            ...
            "Add Tile (transparency)",
            "Reset Tile (clear to background)",
            "Adjust walkable status",
            "Edit Onload",
            "Edit Action",
            "Add Overtile",
            "Copy Tile",
        ...
        list_of_opts = ["Custom", "15x15", "20x20", "25x25", "30x30", "35x35", "40x40"]
        tmp = select_from_list(list_of_opts, True)
        if tmp == -1:
            refresh_map()
            return
